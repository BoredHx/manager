...
export function CalendarGrid({ schedule, onDateClick }: { schedule: Record<string, Shift>; onDateClick: (date: string) => void }) {
  const renderMonth = (monthName: string, monthIndex: number) => {
    ...
    const days = [];
    for (let i = 0; i < firstDay; i++) days.push(<div key={`empty-${monthIndex}-${i}`} className="h-14 bg-muted/20" />);
    for (let d = 1; d <= daysInMonth; d++) {
      const dateKey = `${year}-${(monthIndex + 1).toString().padStart(2, '0')}-${d.toString().padStart(2, '0')}`;
      const shift = schedule[dateKey];
      days.push(
        <div key={dateKey} onClick={() => onDateClick(dateKey)} className={cn("h-14 border p-1 text-[10px] cursor-pointer hover:bg-accent/10 transition-colors relative group", shift?.hasShift ? 'bg-accent/5' : '')}>
          <span className="font-semibold text-muted-foreground group-hover:text-primary">{d}</span>
          {shift?.hasShift && <div className="mt-1 bg-primary text-primary-foreground rounded-sm px-1 overflow-hidden whitespace-nowrap text-[8px] leading-tight">{shift.startTime}<br/>({shift.employees}x)</div>}
        </div>
      );
    }
    return (
  ...
  return <div className="space-y-4 max-h-[calc(100vh-250px)] overflow-y-auto pr-2 scrollbar-thin">{MONTHS_2026.map(m => renderMonth(m.name, m.index))}</div>;
}